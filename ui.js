...
(() => {
  ...
  function ensureReadingActions() {
    ...
    if (!actions) {
      ...
      actions.innerHTML = `
        <button
          type="button"
          class="book-card__open reading-action reading-action--edit"
          data-reading-edit
        >
          この一枚を書き直す
        </button>
        <button
          type="button"
          class="book-card__open reading-action reading-action--delete"
          data-reading-delete
        >
          本棚から外す
        </button>
  ...
  async function deleteEntry(entry) {
    ...
    showConfirm({
      title: "この一枚を本棚から外す？",
      message: `「${entry.text}」を本棚から外します。外したあと元には戻せません。`,
      confirmText: "本棚から外す",
      cancelText: "そのままにする",
      onConfirm: async () => {
