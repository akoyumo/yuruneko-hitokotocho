...
(() => {
  ...
  function getSelectedThemes() {
    ...
    if (newTheme) selected.push(newTheme);
  ...
  function ensureReadingActionStyles() {
    if (document.getElementById("readingActionStylesV16")) return;

    const style = document.createElement("style");
    style.id = "readingActionStylesV16";
    style.textContent = `
      #readingEntryActions {
        display: flex;
        align-items: center;
      ...
      #readingEntryActions .reading-action-button {
        ...
        width: auto;
        padding: 9px 16px;
        border: 1px solid rgba(109, 159, 173, 0.16);
        border-radius: 999px;
        background: #eef8fb;
        color: #33454d;
        font: inherit;
  ...
  async function removeFromBookshelf(entry) {
    ...
    showConfirm({
      title: "この一枚を本棚からしまう？",
      message: `「${entry.text}」を本棚から取り出します。取り出したあとは元に戻せません。`,
      confirmText: "本棚からしまう",
      cancelText: "そのままにする",
      onConfirm: async () => {
