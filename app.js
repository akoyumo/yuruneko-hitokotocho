...
(() => {
  ...
  function openReadingSeat() {
    /*
      「めくる」や「今日の一枚」から読書席を開くときは、
      UI側が持っている最新の一枚をそのまま開きます。
      写真・テーマ・メモも一緒に読書席へ渡します。
    */
    const uiEntries =
      typeof window.YurunekoUI?.getEntries === "function"
        ? window.YurunekoUI.getEntries()
        : [];

    const newestEntry = uiEntries[0] || null;

    if (newestEntry && typeof window.YurunekoUI?.openReading === "function") {
      window.YurunekoUI.openReading(newestEntry);
      return;
    }

    /*
      UIがまだ読み込めていない場合の予備処理。
      文字情報に加えて、写真も読書席へコピーします。
    */
    ...
    if (
      ...
      readingPlaceholder &&
      todayCardImage?.src &&
      !todayCardImage.hidden
    ) {
      readingImage.src = todayCardImage.src;
      readingImage.hidden = false;
      readingPlaceholder.hidden = true;
    } else if (readingImage && readingPlaceholder) {
      readingImage.src = "";
      readingImage.hidden = true;
      readingPlaceholder.hidden = false;
    }
  ...
  function openKaguraSurprise() {
    ...
    try {
      previousId = localStorage.getItem("yuruneko-kagura-last-id") || "";
    } catch (error) {
      console.warn("前回の一枚を読み込めませんでした。", error);
    }

    const candidates =
      uiEntries.length > 1
        ? uiEntries.filter((entry) => entry.id !== previousId)
        : uiEntries;

    const selectedEntry =
      candidates[Math.floor(Math.random() * candidates.length)] || uiEntries[0];

    try {
      localStorage.setItem("yuruneko-kagura-last-id", selectedEntry.id || "");
    } catch (error) {
      console.warn("今回の一枚を記録できませんでした。", error);
    }

    if (typeof window.YurunekoUI?.openReading === "function") {
      window.YurunekoUI.openReading(selectedEntry);
      return;
    }

    showNotice(
      "神楽のお楽しみ箱",
      "一枚は選べたけれど、読書席を開けませんでした。もう一度試してね。"
    );
  }
